...
ALTER TABLE FACTURACION
   DROP CONSTRAINT IF EXISTS FK_FACTURACION_MUNICIPIO;
...
-- Tabla INFORME_MANTENIMIENTO
ALTER TABLE INFORME_MANTENIMIENTO
   DROP CONSTRAINT IF EXISTS FK_VENTA_INFORME_MANTENIMIENTO;
...
DROP INDEX IF EXISTS MUNICIPIO_DEPARTAMENTO_FK;
...
DROP TABLE IF EXISTS INFORME_MANTENIMIENTO CASCADE;
...
-- Índice FACTURACION_MUNICIPIO_FK
CREATE INDEX FACTURACION_MUNICIPIO_FK ON FACTURACION (ID_MUNICIPIO);
...
-- Definición de tabla INFORME_MANTENIMIENTO
CREATE TABLE INFORME_MANTENIMIENTO
(
   ID_INFORME_MANTENIMIENTO SERIAL PRIMARY KEY,
   ID_VENTA INTEGER,
   FECHA_PAGO DATE,
   FECHA_CUOTA DATE,
   LOTE VARCHAR(20),
   ...
   MONTO_AJUSTE DOUBLE PRECISION,
);
-- Índice VENTA_INFORME_MANTENIMIENTO_FK
CREATE INDEX VENTA_INFORME_MANTENIMIENTO_FK ON INFORME_MANTENIMIENTO (ID_VENTA);
...
(
   ...
   ID_TERRENO INTEGER,
   FECHA DATE,
   FECHA_CORTE_MANTENIMIENTO DATE,
   FECHA_CORTE_FINANCIAMIENTO DATE,
   NOMBRE VARCHAR(200),
   PRECIO NUMERIC(9,2),
   DESCUENTO NUMERIC(9,2),
   MONTO NUMERIC(9,2),
...
-- Definición de tabla DEPARTAMENTO
...
-- Índice MUNICIPIO_DEPARTAMENTO_FK
CREATE INDEX MUNICIPIO_DEPARTAMENTO_FK ON MUNICIPIO (ID_DEPARTAMENTO);
...
ALTER TABLE FACTURACION
ADD CONSTRAINT FK_FACTURACION_MUNICIPIO FOREIGN KEY (ID_MUNICIPIO)
...
-- Tabla INFORME_MANTENIMIENTO
ALTER TABLE INFORME_MANTENIMIENTO
ADD CONSTRAINT FK_VENTA_INFORME_MANTENIMIENTO FOREIGN KEY (ID_VENTA)
REFERENCES VENTA (ID_VENTA);
...
SELECT 
    ...
    MATRICULA,
    CASE
        WHEN NUMERO < 10 THEN '0' || CAST(NUMERO AS TEXT)
        ELSE NUMERO::TEXT
    END || SECCION AS LOTE,
    POLIGONO,
    AREA_METROS,
...
SELECT
    ...
    T.POLIGONO, 
    CASE
        WHEN T.NUMERO < 10 THEN '0' || T.NUMERO::TEXT
        ELSE T.NUMERO::TEXT
    END || T.SECCION AS LOTE,
    T.MATRICULA,
    T.AREA_METROS,
    T.AREA_VARAS
FROM
...
CREATE OR REPLACE FUNCTION trg_agregar_propietario() RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION trg_agregar_propietario();

-- TRIGGER TRG_ELIMINAR_PROPIETARIO
CREATE OR REPLACE FUNCTION trg_eliminar_propietario() RETURNS TRIGGER AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION trg_eliminar_propietario();

-- TRIGGER TRG_AGREGAR_VISITANTE
CREATE OR REPLACE FUNCTION trg_agregar_visitante() RETURNS TRIGGER AS $$
DECLARE
  id_lista_documento INTEGER;
BEGIN
  INSERT INTO listado_documento DEFAULT VALUES RETURNING id_list_documento INTO NEW.id_list_documento;
...
FOR EACH ROW
EXECUTE FUNCTION trg_agregar_visitante();

-- TRIGGER TRG_ELIMINAR_VISITANTE
CREATE OR REPLACE FUNCTION trg_eliminar_visitante() RETURNS TRIGGER AS $$
DECLARE
  id_lista_documento INTEGER;
BEGIN
  id_lista_documento := OLD.id_list_documento;
...
FOR EACH ROW
EXECUTE FUNCTION trg_eliminar_visitante();
